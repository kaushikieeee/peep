  ...
  if (submitted) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center px-4 text-center">
        ...
        <p className="text-white/60 mb-6">Your evidence is now public and visible to local authorities. Thank you for keeping us informed.</p>
        <Link href="/peep/user" className="w-full py-2 px-4 rounded-lg font-semibold text-white text-center" style={{ backgroundColor: 'var(--peep-primary)' }}>
          Back to Home
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-white/20 backdrop-blur-xl bg-white/5">
        <Link href="/peep/user/report-flow/describe-issue" className="p-2 hover:bg-white/10 rounded-lg">
          <ChevronLeft className="w-5 h-5 text-white" />
        </Link>
        <h1 className="text-lg font-semibold text-white">Review Report</h1>
        <div className="w-10" />
      </div>
      ...
      <div className="flex-1 px-4 py-2 space-y-2 overflow-y-auto">
        ...
        {formData.title && (
          <div className="p-2 bg-white/5 rounded-lg border border-white/10">
            <p className="text-xs text-white/60 mb-1">Report Title</p>
            <p className="font-semibold text-white text-sm">{formData.title}</p>
          </div>
        )}
        ...
        {formData.location && (
          <div className="p-2 bg-white/5 rounded-lg border border-white/10">
            ...
            <p className="text-xs text-white">Lat: {formData.location.lat.toFixed(4)}, Lng: {formData.location.lng.toFixed(4)}</p>
          </div>
        )}
        ...
        {formData.category && (
          <div className="p-2 bg-white/5 rounded-lg border border-white/10">
            <p className="text-xs text-white/60 mb-1">Problem Type</p>
        ...
        {formData.severity && (
          <div className="p-2 bg-white/5 rounded-lg border border-white/10">
            <p className="text-xs text-white/60 mb-1">Impact Level</p>
            <div className="flex items-center gap-2">
              ...
              />
              <p className="font-semibold text-white text-sm">{formData.severity}</p>
            </div>
          </div>
        )}
      </div>
      ...
      {/* Submission buttons */}
      <div className="px-4 py-2 space-y-1 border-t border-white/20">
        <button
          onClick={handleSubmit}
          className="w-full py-2 px-4 rounded-lg font-semibold text-white text-sm"
          style={{ backgroundColor: 'var(--peep-primary)' }}
        >
          Submit Report
        </button>
        <Link href="/peep/user" className="block w-full py-2 px-4 rounded-lg font-semibold border-2 border-white/20 text-white text-center hover:bg-white/10 text-sm">
          Cancel
        </Link>
      </div>
    </div>
